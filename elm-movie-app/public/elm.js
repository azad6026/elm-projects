...
var $author$project$Main$GotMovies = function (a) {
	return {$: 'GotMovies', a: a};
};
...
var $author$project$Main$Movie = F4(
	function (id, title, poster_path, overview) {
		return {id: id, overview: overview, poster_path: poster_path, title: title};
	});
...
var $author$project$Main$movieDecoder = A5(
	...
	A2($elm$json$Json$Decode$field, 'poster_path', $elm$json$Json$Decode$string),
	A2($elm$json$Json$Decode$field, 'overview', $elm$json$Json$Decode$string));
var $author$project$Main$getPopularMovies = function () {
	var url = 'https://api.themoviedb.org/3/movie/popular?api_key=fcd88911f1a3d29acb72510ad5c4413c&page=1';
	var decoder = A2(
	...
	return $elm$http$Http$get(
		{
			expect: A2($elm$http$Http$expectJson, $author$project$Main$GotMovies, decoder),
			url: url
		});
}();
...
var $author$project$Main$update = F2(
	function (msg, model) {
		switch (msg.$) {
			...
			case 'ToggleFavorite':
				var id = msg.a;
				var isFavorite = A2($elm$core$List$member, id, model.favorites);
				var updatedFavorites = isFavorite ? A2(
					$elm$core$List$filter,
					$elm$core$Basics$neq(id),
					model.favorites) : A2($elm$core$List$cons, id, model.favorites);
				return _Utils_Tuple2(
					_Utils_update(
						model,
	...
	switch (_v0) {
		...
		case 'favorites':
			return A2(
				$elm$core$List$filter,
				function (movie) {
					return A2($elm$core$List$member, movie.id, model.favorites);
				},
				model.movies);
		default:
			return model.movies;
...
var $author$project$Main$movieCard = F2(
	function (favorites, movie) {
		var isFavorite = A2($elm$core$List$member, movie.id, favorites);
		var imageUrl = 'https://image.tmdb.org/t/p/w200' + movie.poster_path;
		return A2(
			$elm$html$Html$figure,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('movie-card')
				]),
			_List_fromArray(
				[
					A2(
					...
					_List_fromArray(
						[
							...
							$elm$html$Html$text(
							'(' + (A3($elm$core$String$slice, 0, 4, movie.overview) + ')')),
							A2(
							$elm$html$Html$button,
							_List_fromArray(
								[
									$elm$html$Html$Events$onClick(
									$author$project$Main$ToggleFavorite(movie.id)),
									$elm$html$Html$Attributes$class('favorite-button')
								]),
							_List_fromArray(
								[
									$elm$html$Html$text(
				...
				if (model.isLoading) {
					...
					if (_v0.$ === 'Just') {
						...
						return A2(
							...
							A2(
								$elm$core$List$map,
								$author$project$Main$movieCard(model.favorites),
								$author$project$Main$getFilteredMovies(model)));
					}
				}
			}()
			]));
